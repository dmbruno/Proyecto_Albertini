...
export default function EditarPedido() {
  ...
  useEffect(() => {
    cargar(id).then(({ pedido, items, error }) => {
      ...
      items.forEach(item => {
        ...
        if (!byCliente[cid]) {
          byCliente[cid] = {
            ...
            clienteId: cid,
            listaId:   item.clientes?.lista_precios_id ?? '',
            items:     [],
          }
        }
      ...
      setSecciones(loaded.length > 0 ? loaded : [{ tempId: crypto.randomUUID(), clienteId: '', listaId: '', items: [newItem()] }])
      setLoadingData(false)
    })
  }, [id])

  /* ── sección ── */
  const addSeccion    = () => setSecciones(prev => [...prev, { tempId: crypto.randomUUID(), clienteId: '', listaId: '', items: [newItem()] }])
